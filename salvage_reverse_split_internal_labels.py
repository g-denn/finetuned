#!/usr/bin/env python3
"""Salvage ordinary reverse-split flagged rows with split-aware sanity checks."""
...
VALIDATION_IN = BASE_DIR / "validation_results_with_delisted_archive_symbol_repair.csv"
TRAINING_READY_IN = BASE_DIR / "training_ready_after_delisted_archive_symbol_repair.csv"

VALIDATION_OUT = BASE_DIR / "validation_results_with_reverse_split_salvage.csv"
TRAINING_READY_OUT = BASE_DIR / "training_ready_after_reverse_split_salvage.csv"
ROW_REVIEWS_CSV = BASE_DIR / "reverse_split_salvage_reviews.csv"
SUMMARY_JSON = BASE_DIR / "reverse_split_salvage_summary.json"
...
def load_payload(symbol: str) -> dict[str, Any]:
    path = BASE_DIR / "symbol_cache" / safe_symbol_filename(symbol)
    if not path.exists():
        return {}
    try:
...
def split_aware_sanity(payload: dict[str, Any], reviews: dict[str, dict[str, Any]]) -> tuple[bool, str, dict[str, Any]]:
    ...
    return True, "split_adjusted_continuity_passed", diagnostics
...
def review_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    if "reverse_split_provider_adjusted" not in flags:
        return {**base, "review_status": "manual_review", "training_action": "hold", "reason": "not_reverse_split_flagged", "passed_horizons": {}}
    ...
    if normal_ok:
        return {
            **base,
            ...
            "reason": "reverse_split_flag_outside_validated_window_internal_sanity_passed",
    ...
    return {
        **base,
        "review_status": "pass",
        "training_action": "add_to_training_ready",
        "reason": "reverse_split_event_adjusted_continuity_passed",
        "passed_horizons": {horizon: review["multiplier"] for horizon, review in reviews.items()},
        "split_sanity": split_diag,
        "confidence": 0.61,
    }
...
def select_candidates(rows: list[dict[str, str]], training_keys: set[tuple[str, str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        if row_key(row) in training_keys:
            continue
        if "reverse_split_provider_adjusted" in split_flags(row):
...
def write_validation(rows: list[dict[str, str]], reviews_by_key: dict[tuple[str, str, str], dict[str, Any]]) -> None:
    extra = [
        "reverse_split_salvage_status",
        "reverse_split_salvage_action",
        "reverse_split_salvage_reason",
        "reverse_split_salvage_passed_horizons",
    ]
    ...
    with VALIDATION_OUT.open("w", newline="", encoding="utf-8") as handle:
        ...
        for row in rows:
            ...
            if review:
                output.update(
                    {
                        "reverse_split_salvage_status": review.get("review_status"),
                        "reverse_split_salvage_action": review.get("training_action"),
                        "reverse_split_salvage_reason": review.get("reason"),
                        "reverse_split_salvage_passed_horizons": scalar(review.get("passed_horizons")),
                    }
                )
        ...
        output.update(
            {
                ...
                "math_validation_status": "reverse_split_internal_verified",
                "review_stage": "reverse_split_eodhd_internal_sanity",
                "training_readiness": "training_ready",
                "review_status": "pass",
                "reviewed_at": now,
                "agent_c_status": "pass",
                "agent_c_reason": scalar(review.get("reason")),
                "agent_c_outcome_type": "reverse_split_internal_verified",
                "source_count": "5",
...
def main() -> int:
    ...
    summary = {
        ...
        "input_rows": len(rows),
        "reverse_split_candidates": len(candidates),
