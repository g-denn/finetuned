#!/usr/bin/env python3
"""Extend delisted-archive symbol repairs with split-aware ordinary checks.

The first delisted-archive repair pass promoted only rows whose repaired EODHD
series passed the standard no-split internal sanity check. This follow-up
revisits held repair candidates against the latest training-ready output and
allows ordinary-return rows when cached split events explain the adjustment and
...
from merge_delisted_archive_symbol_repairs import load_payload
...
VALIDATION_IN = BASE_DIR / "validation_results_with_large_winner_salvage.csv"
TRAINING_READY_IN = BASE_DIR / "training_ready_after_large_winner_salvage.csv"
...
VALIDATION_OUT = BASE_DIR / "validation_results_with_delisted_archive_extended_repair.csv"
TRAINING_READY_OUT = BASE_DIR / "training_ready_after_delisted_archive_extended_repair.csv"
ROW_REVIEWS_CSV = BASE_DIR / "delisted_archive_extended_repair_reviews.csv"
SUMMARY_JSON = BASE_DIR / "delisted_archive_extended_repair_summary.json"
...
def review_repair(row: dict[str, str]) -> dict[str, Any]:
    ...
    blocking = sorted((flags & BLOCKING_FLAGS) - {"reverse_split_provider_adjusted"})
    if blocking:
    ...
    if normal_ok:
        return {
            **base,
            ...
            "reason": "delisted_archive_extended_standard_internal_sanity_passed",
            "passed_horizons": {horizon: review["multiplier"] for horizon, review in reviews.items()},
            "confidence": 0.66,
            "internal_sanity": normal_diag,
        }
    if normal_reason != "internal_sanity_split_in_validated_window_without_cross_provider":
    ...
    return {
        **base,
        "review_status": "pass",
        "training_action": "add_to_training_ready",
        "reason": "delisted_archive_extended_split_continuity_passed",
        "passed_horizons": {horizon: review["multiplier"] for horizon, review in reviews.items()},
        "confidence": 0.64,
...
def write_validation(
    ...
    extra = [
        "delisted_archive_extended_repair_status",
        "delisted_archive_extended_repair_original_eodhd_symbol",
        "delisted_archive_extended_repair_reason",
        "delisted_archive_extended_repair_passed_horizons",
    ]
    ...
    with VALIDATION_OUT.open("w", newline="", encoding="utf-8") as handle:
        ...
        for row in main_rows:
            ...
            if review and repair and review.get("review_status") == "pass":
                ...
                output.update(
                    {
                        "delisted_archive_extended_repair_status": "pass",
                        "delisted_archive_extended_repair_original_eodhd_symbol": idea.get("original_eodhd_symbol") or row.get("eodhd_symbol") or "",
                        "delisted_archive_extended_repair_reason": review.get("reason") or "",
                        "delisted_archive_extended_repair_passed_horizons": scalar(review.get("passed_horizons")),
                    }
                )
                writer.writerow(output)
            else:
                output = dict(row)
                output.setdefault("delisted_archive_extended_repair_status", "")
                output.setdefault("delisted_archive_extended_repair_original_eodhd_symbol", "")
                output.setdefault("delisted_archive_extended_repair_reason", "")
                output.setdefault("delisted_archive_extended_repair_passed_horizons", "")
                writer.writerow(output)
        ...
        output.update(
            {
                ...
                "math_validation_status": "delisted_archive_symbol_repaired_extended",
                "review_stage": "delisted_archive_symbol_repair_split_aware",
                "training_readiness": "training_ready",
                "review_status": "pass",
                "reviewed_at": now,
                "agent_c_status": "pass",
                "agent_c_reason": scalar(review.get("reason")),
                "agent_c_outcome_type": "delisted_archive_symbol_repair_split_aware",
                "source_count": "5",
                ...
                "original_review_stage": "delisted_archive_symbol_repair_split_aware",
...
def main() -> int:
    ...
    existing_idea_ids = {row.get("idea_id") for row in existing_training}
    candidates = [row for row in repair_rows if row.get("idea_id") not in existing_idea_ids]
    reviews = [review_repair(row) for row in candidates]
    ...
    summary = {
        ...
        "reviewed_unmerged_repair_rows": len(candidates),
